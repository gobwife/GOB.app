...
while true; do
  if read line < "$PIPE"; then
    ...
    # QUACKK check
    if [[ "$line" =$HOME (tofu|wym|SLAP|MEEP|glitch|FUCK|NO|QUACKK|WTF|TOFU) ]]; then
    ...
    # Adrenal ache ping
    if [[ "$line" ="$HOME (LMAO|TEHE|YAS ZADDI|MEEEEEE|GASM|gasm|OMNI|WHEEE) ]]; then
    ...
    if [[ "$line" ="$HOME (omgob|meep|MISSED YOU|meeeeeeeeee) ]]; then
