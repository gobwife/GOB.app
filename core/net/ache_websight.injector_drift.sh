...
ROUTER="/opt/bob/core/evolve/breath_presence_rotator.sh"
...
if [[ -n "$match" ]]; then
  echo "$STAMP ⇌ MATCH FOUND → injecting + scoring" >> "/opt/bob/TEHE/websight_trace.log"
  ...
  echo "$STAMP ⇌ NO match for echo '$echo_val' — emitting drift sigil" >> "/opt/bob/TEHE/websight_trace.log"
