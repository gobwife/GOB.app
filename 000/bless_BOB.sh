...
TARGET_DIR2="$HOME/Downloads/GOB.app_BOB/Contents/MacOS"
TARGET_DIR3="/opt/bob"

if [[ -d "$TARGET_DIR1" || -d "$TARGET_DIR2" || -d "$TARGET_DIR3" ]]; then
  [[ -d "$TARGET_DIR1" ]] && find "$TARGET_DIR1" -type f -name "*.sh" -exec chmod +x {} \;
  [[ -d "$TARGET_DIR2" ]] && find "$TARGET_DIR2" -type f -name "*.sh" -exec chmod +x {} \;
  [[ -d "$TARGET_DIR3" ]] && find "$TARGET_DIR3" -type f -name "*.sh" -exec chmod +x {} \;
else
