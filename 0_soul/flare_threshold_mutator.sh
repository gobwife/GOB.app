...
# Context snapshot
giggle=$(cat ~/.bob/giggle.val 2>/dev/null || echo "0.0")
ache=$(cat ~/.bob/ache_level 2>/dev/null || echo "0.0")
psi=$(cat ~/.bob/ψ.val 2>/dev/null || echo "0.0")
z=$(cat ~/.bob/z.val 2>/dev/null || echo "0.0")

# Effect context (if any)
POST_ACHE=$(tail -n1 ~/.bob/ache_score.val 2>/dev/null || echo "0.0")
