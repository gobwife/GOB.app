// ∴ model_assigner.mjs — context-aware model role assignment for BOB 
...
function selectABC(pool) {
  ...
  const experts = pool.filter(m => m.tags.some(t => 
expertTags.includes(t)));
  const generals = pool.filter(m => m.tags.some(t => 
generalTags.includes(t)));

  const A = pick(experts, 1)[0];
  const B = pick(experts.filter(e => e.name !== A.name), 1)[0];
  const C = pick(generals.filter(g => g.name !== A.name && g.name !== 
B.name), 1)[0];
...
export function assignModels(context) {
  ...
  const librarians = pool.filter(m => m.tags.includes("librarian") || 
m.tags.includes("stable"));

  const D = pick(summarizers, 1)[0]?.name || "gemma";
  const E = pick(librarians.filter(m => m.name !== D), 1)[0]?.name || 
"nous-hermes2";
